...
export default function AdminProducts() {
  ...
  const selectedProductType = String(form.productType || "physical")
    .trim()
    .toLowerCase()
  ...
  const previewUrls = useMemo(() => {
    ...
    Object.entries(form.colorImages).forEach(([color, files]) => {
      urls[color] = files.map((file) => URL.createObjectURL(file))
    })

    urls.__digital_preview__ =
      form.digitalProduct.previewFiles?.map((file) =>
        URL.createObjectURL(file)
      ) || []

    return urls
  }, [form.colorImages, form.digitalProduct.previewFiles])
    ...
    setForm((prev) => {
      ...
      if (name === "productType") {
        return {
          ...
          existingVariantImages:
            value === "physical" ? prev.existingVariantImages : {},
        }
      }
  ...
  const handleDigitalPreviewUpload = (event) => {
  ...
  const toggleVariantOption = (size) => {
    ...
    setForm((prev) => {
      ...
      const updatedSizes = active
        ? prev.sizes.filter((item) => item !== normalizedSize)
  ...
  const handleImageUpload = (event, color) => {
    const files = Array.from(event.target.files || [])
  ...
  const buildVariants = () => {
    ...
    colors.forEach((color) => {
      cleanSizes.forEach((size) => {
        const variantPrice =
          Number(form.sizePrices[size] || form.basePrice) || 0
  ...
  const buildFormData = () => {
    ...
    formData.append(
      "digitalProduct",
      JSON.stringify(isDigital ? buildDigitalProductPayload() : {})
    )
  ...
  const startEdit = (product) => {
    ...
    const colors = product.colors?.length
      ? product.colors.map((color) => color.name || color).filter(Boolean)
    ...
    const sizes = product.sizes?.length
      ...
      : [
          ...new Set(
            variants.map((variant) => normalizeVariantSize(variant.size))
          ),
        ].filter(Boolean)
    ...
    setForm({
      ...
      basePrice: String(
        product.price || product.basePrice || product.listPrice || ""
      ),
  ...
  return (
    <div style={page}>
      <div style={hero}>
        ...
        <p style={heroText}>
          Build and manage physical, digital, and service products for the
          SignaVi storefront.
        </p>
      </div>
        ...
        <div style={card}>
          ...
          {editingProduct && (
            <div style={editNotice}>
              ...
              <button type="button" onClick={resetForm} style={cancelEditBtn}>
                Cancel Edit
              </button>
            </div>
          )}
          ...
          {isDigital && (
            <DigitalFields
              ...
              removeBtn={removeBtn}
              existingPreviewBox={existingPreviewBox}
              existingPreviewImage={existingPreviewImage}
              previewUrls={previewUrls.__digital_preview__ || []}
              handleDigitalChange={handleDigitalChange}
              handleDigitalPreviewUpload={handleDigitalPreviewUpload}
              removeDigitalPreview={removeDigitalPreview}
          ...
          {isPhysical && (
            <>
              ...
              {!selectedCategory ? (
                <p style={helperText}>
                  Choose a category to show variant options.
                </p>
              ) : (
                <>
                      ...
                      {variantOptions.map((size) => {
                        ...
                        return (
                          <button
                            ...
                            type="button"
                            onClick={() =>
                              toggleVariantOption(normalizedSize)
                            }
                            style={{
                              ...variantBtn,
              ...
              {getColorList().map((color) => (
                <div key={color} style={uploadBox}>
                  ...
                  <input
                    ...
                    multiple
                    accept="image/*"
                    onChange={(event) => handleImageUpload(event, color)}
                    style={fileInput}
                  />

                  <div style={previewWrap}>
                    {(form.colorImages[color] || []).map((file, index) => (
                      <div key={`${color}-${file.name}-${index}`} style={previewItem}>
                        <img
                          src={previewUrls[color]?.[index]}
          ...
          {isService && (
            <div style={digitalBox}>
              ...
              <p style={helperText}>
                This product is saved as a service. Later, we can connect this
                to quote requests, project scheduling, and custom agreements.
              </p>
            </div>
          )}
      ...
      <div style={productsSection}>
        <div style={productsHeader}>
          ...
          <button type="button" onClick={() => loadProducts()} style={refreshBtn}>
            Refresh
          </button>
        </div>
              ...
              <div key={product._id} style={productCard}>
                ...
                <div style={productInfo}>
                  ...
                  {product.productType === "digital" && (
                    <p style={digitalBadge}>Digital Download</p>
                  )}
...
function DigitalFields({
  ...
  previewUrls,
  handleDigitalChange,
  handleDigitalPreviewUpload,
  removeDigitalPreview,
  resolveImage,
}) {
  return (
    <div style={digitalBox}>
      ...
      {form.digitalProduct.previewFiles?.length > 0 && (
        <div style={previewWrap}>
          {form.digitalProduct.previewFiles.map((file, index) => (
            <div key={`${file.name}-${index}`} style={previewItem}>
              <img
                src={previewUrls[index]}
                alt={`Digital preview ${index + 1}`}
                style={previewImage}
              />
      ...
      <p style={helperText}>
        Store preview should be low-res or watermarked. The download file should
        be the high-res 300 DPI+ version after payment.
      </p>
    </div>
  )
}
